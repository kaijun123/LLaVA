...
def load_model(model_path, model_base, model_name, vision_tower_path, image_processor_path, device="cuda"):
    ...
    Ensure that "lora" exists within the model_path provided so that the lora weights will be merged when loading the model weights

    vision_tower_path and image_processor_path are provided to instantiate the VLM with a custom vision encoder. It depends on local changes made to the LlavaLlamaForCausalLM code.
    Instantiating the model using other classes eg LlavaMptForCausalLM, LlavaMistralForCausalLM, AutoModelForCausalLM will not have this effect
    """
...
# v1(conv_vicuna_v1) is used in this work, with deliberate reference to llava-v1.5 training scripts.
